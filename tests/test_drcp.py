...
from oracle.tns import encode_dictionary_description, decode_packet
from oracle.tns_consts import TTI_SVR_PIGGYBACK, TTI_STA, PURITY_SELF


def _env(**extra):
    base = {'host': 'h', 'port': 1521, 'user': 'u', 'service_name': 'svc',
            'app_name': 'pyoracle'}
    base.update(extra)
    return {'env': base}
...
class TestDescriptor(unittest.TestCase):
    def test_pooled_when_drcp(self):
        self.assertIn(b"(SERVER=POOLED)",
                      encode_dictionary_description(_env(cclass='APP')))
        self.assertIn(b"(SERVER=POOLED)",
                      encode_dictionary_description(_env(purity=PURITY_SELF)))

    def test_no_pooled_otherwise(self):
        self.assertNotIn(b"(SERVER=POOLED)",
                         encode_dictionary_description(_env()))


class TestParams(unittest.TestCase):
    def test_connection_threads_cclass_purity(self):
        c = OracleConnect(host='x', port=1, user='u', password='p',
                          cclass='MYAPP', purity=PURITY_SELF)
...
class TestPiggyback(unittest.TestCase):
    def test_skips_sess_ret(self):
        ...
        self.assertEqual(decode_packet(pkt, (None, None, [])),
                         (True, (None, None, [])))
